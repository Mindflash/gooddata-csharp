...
namespace GoodData.API.Api {
	...
	public class ApiWrapper : ApiWrapperBase {
		...
		public string CreateProject(string title, string summary, string template = null, string driver = SystemPlatforms.PostGres) {
			CheckAuthentication();
			var url = Url.Combine(Config.ServiceUrl, Constants.PROJECTS_URI);
		...
		public void DeleteProject(string projectId) {
			CheckAuthentication();
			var url = Url.Combine(Config.ServiceUrl, Constants.PROJECTS_URI, projectId);
			DeleteRequest(url);
		}
		...
		public void DeleteObject(string projectId, string relativeUri) {
			CheckAuthentication();
			var url = Url.Combine(Config.ServiceUrl, relativeUri);
			DeleteRequest(url);
		}
		...
		public string ImportProject(string projectId, string token) {
			CheckAuthentication();
			var url = Url.Combine(Config.ServiceUrl, Constants.MD_URI, projectId, Constants.PROJECT_IMPORT_URI);
		...
		public List<AccountResponseSettingWrapper> GetDomainUsers(string domain = "") {
			...
			var url = Url.Combine(Config.ServiceUrl, Constants.DOMAIN_URI, domain, Constants.DOMAIN_USERS_SUFFIX);
